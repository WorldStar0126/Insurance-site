...
export const goProductCollapse = (id, flag) => {
    const axios = require('axios');
    document.querySelectorAll(".product-collapse")[0].classList.add("show");
    http.get(`/get/productbyid?id=${id}`).then(resp => {
        document.querySelectorAll(".select-pro-cat")[0].value = parseInt(resp.data.n_catid);
        document.querySelectorAll(".product-name")[0].value = resp.data.c_productname;
        document.querySelectorAll(".select-pro-company")[0].value = parseInt(resp.data.n_companyid);
        document.querySelectorAll(".select-pro-subcat")[0].value = parseInt(resp.data.n_subcatid);
    });
    if (flag && flag == true) {
...
export const goPolicyCollapse = (id, ...props) => {
